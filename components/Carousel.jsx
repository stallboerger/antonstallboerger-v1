...
const Carousel = () => {
  ...
  return (
    <div className="relative pb-6 z-0">
      ...
      <button onClick={handlePreviousClick} className="absolute left-0 top-[42%] flex justify-between items-center ml-4 rounded-full bg-zinc-50/75 p-2" aria-label="Left">
        <NavArrowLeft className="h-6 w-6 stroke-2 text-zinc-800" />
      </button>
      <button onClick={handleNextClick} className="absolute right-0 top-[42%] flex justify-between items-center mr-4 rounded-full bg-zinc-50/75 p-2" aria-label="Right">
